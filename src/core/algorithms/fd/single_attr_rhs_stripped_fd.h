...
struct SingleAttrRhsStrippedFd {
    ...
    model::FunctionalDependency ToFd(model::TableHeader const& table_header_, model::Index rhs) const {
        ...
        return {table_header_.table_name,
                std::move(lhs_attrs),
                {{table_header_.column_names[rhs], rhs}}};
    }
};
}  // namespace algos

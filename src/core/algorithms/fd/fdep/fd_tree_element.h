...
#include "core/algorithms/fd/lhs_lim_fd_storage_builder.h"
...
public:
    ...
    void CreateAnswer(std::size_t attr_num, algos::LhsLimFdStorageBuilder& builder,
...
private:
    ...
    void TransformTreeFdCollection(
            boost::dynamic_bitset<>& lhs, algos::LhsLimFdStorageBuilder& builder,
            unsigned int max_lhs = std::numeric_limits<unsigned int>::max()) const;
};

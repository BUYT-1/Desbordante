...
inline static boost::dynamic_bitset<> IntToBitset(pybind11::int_ py_int, std::size_t const size) {
    ...
    int is_negative = PyLong_IsNegative(py_int.ptr());
    if (is_negative == -1) throw pybind11::error_already_set();
    if (is_negative) {
        throw pybind11::value_error("Unexpected negative number as bit storage: " +
                                    static_cast<std::string>(pybind11::str(py_int)));
    }
    static constexpr std::size_t block_digits = std::numeric_limits<unsigned long long>::digits;
    pybind11::int_ const block_digits_py = pybind11::int_{block_digits};
